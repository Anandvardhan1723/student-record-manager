...
Session(app)
mydb=mysql.connector.connect(host="localhost",user="root",password="Anand@1722",db="spm")
app.secret_key=b'\xe9\xcc\x86Q\xc2\x1d'
@app.route("/")
def index():
...
def register():
    if request.method=="POST":
        ...
        cursor.execute("select count(email) from student where email=%s",[mail])
        data=cursor.fetchone()[0]
        if data==0:
            otp=genotp()
            data={"otp":otp,"mail":mail,"fname":fname,"lname":lname,"pwd":pwd}
            subject="Verification otp for Application"
...
def verifyotp(data1):
    try:
        ...
        if request.method=="POST":
            ...
            if uotp==data1["otp"]:
                cursor=mydb.cursor(buffered=True)
                cursor.execute("insert into student(email,stu_fname,stu_lname,password) values(%s,%s,%s,%s)",[data1["mail"],data1["fname"],data1["lname"],data1["pwd"]])
                mydb.commit()
                cursor.close()
